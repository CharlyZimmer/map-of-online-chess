...
def get_country_dfs(openings_df: DataFrame) -> Tuple[DataFrame, DataFrame]:
    ...
    opening_mean = openings_df.groupby('matched_id')['share'].mean().rename('mean')
    opening_std = openings_df.groupby('matched_id')['share'].std().rename('std')
    ...
    country_opening_df = openings_df.groupby(['country', 'matched_id'])['share'].mean().reset_index()
    ...
    tmp_df = merge(opening_mean_std, country_opening_df, on='matched_id', how='left')
