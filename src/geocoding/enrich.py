...
class EnrichGeoJSON:
    def __init__(self, country_openings_parquet: str = "test_cleaned_openings.parquet"):
        ...
        openings_path = DATA_DIRECTORY / f"openings"
        ...
        self.openings_path = openings_path / "Full_Year_mean_std.parquet"
    ...
    def get_color_positions(
        self, max_val: float = 3.0, min_val: float = -1.0, num_positions: int = 400
    ):
        ...
        self.meta_json["positive_positions"] = round(
            abs(max_val) / value_range * num_positions
        )
        self.meta_json["negative_positions"] = round(
            abs(min_val) / value_range * num_positions
        )

        suffixes = ["".join(tup) for tup in itertools.product(["", "won_"], ["w", "b"])]
        for s in suffixes:
            self.country_df[f"pos_{s}"] = self.country_df[f"stand_p_{s}"].apply(
                lambda p: self._get_position(
                    p=p,
                    min_val=min_val,
                    step_size=step_size,
    ...
    def add_openings(self):
        known_openings = (
            self.country_df.groupby("matched_id")["matched_id"].count().index.tolist()
        )
        df = OpeningLoader().df
        metadata = {}
        ...
        for opening in tqdm(known_openings, desc=f"Adding openings to geojson"):
            ...
            for s in suffixes:
                opening_dict = {
                    ...
                    for country, position, probability in zip(
                        opening_df.country, opening_df[f"pos_{s}"], opening_df[f"p_{s}"]
                    )
                }
    ...
    def add_mean_and_stddev(self):
        openings_df = read_parquet(self.openings_path)
        for index, row in openings_df.iterrows():
            for column in openings_df:
                if not row["matched_id"] in self.meta_json["openings"]:
                    continue
                if not column == "matched_id":
                    self.meta_json["openings"][row["matched_id"]][column] = row[column]

    def _load_and_filter_df(self):
        # 1. Load the parquet files
        country_df = read_parquet(self.country_openings_path)
        player_counts = read_parquet(
            str(self.country_openings_path).replace(".p", "_player_count.p")
        )
        ...
        country_df["country"] = country_df["country"].apply(
            lambda iso_a2: map_dict.get(iso_a2, None)
        )

        player_counts["country"] = player_counts["country"].apply(
            lambda iso_a2: map_dict.get(iso_a2, None)
        )
    ...
    def _load_json_files(self):
        if not os.path.isfile(self.geojson_path):
            ...
            print(
                f"No file {self.geojson_path} found. Downloading countries.geojson to the specified path..."
            )
            _ = wget.download(
